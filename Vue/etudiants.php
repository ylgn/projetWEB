<!DOCTYPE html>
<html>
<head>
	<title>Etudiants</title>
</head>
<body>
	<nav>
		<?php
			include "template.php";
		?>
	</nav>

	<div id="index" class="" style="background-image:url('img/studentsbg.jpg');background-size: auto 100%; background-repeat: no-repeat; background-size: cover; background-position: center center;">

		<div class="mask rgba-gradient d-flex justify-content-center align-items-center">
			<div class="container">
				<div class="row myRow mb-5">
					<div class=" col mt-xl-5 mb-5 ml-5">
						<h1 class="h1-responsive font-weight-bold h1-xs">Les étudiants inscrits en M1 MIAGE</h1>

						<hr class="myHr">

						<div class="card-columns mr-5">
							<div class="card mr-5">
								<img class="card-img-top " src="img/tete1.png" alt="Card image">
								<div class="card-body">
									<h4 class="card-title">John Doe</h4>
									<p class="card-text">Spécialité du stage</p>
									<a href="#" class="btn btn-primary">Page personnelle</a>
								</div>
							</div>
							<div class="card mr-5">
								<img class="card-img-top " src="img/tete2.png" alt="Card image">
								<div class="card-body">
									<h4 class="card-title">John Doe</h4>
									<p class="card-text">Spécialité du stage</p>
									<a href="#" class="btn btn-primary">Page personnelle</a>
								</div>
							</div>
							<div class="card mr-5">
								<img class="card-img-top " src="img/tete3.png" alt="Card image">
								<div class="card-body">
									<h4 class="card-title">John Doe</h4>
									<p class="card-text">Spécialité du stage</p>
									<a href="#" class="btn btn-primary">Page personnelle</a>
								</div>
							</div>
							<div class="card mr-5">
								<img class="card-img-top " src="img/tete4.png" alt="Card image">
								<div class="card-body">
									<h4 class="card-title">John Doe</h4>
									<p class="card-text">Spécialité du stage</p>
									<a href="#" class="btn btn-primary">Page personnelle</a>
								</div>
							</div>
							<div class="card mr-5">
								<img class="card-img-top " src="img/matieresbg.jpg" alt="Card image">
								<div class="card-body">
									<h4 class="card-title">John Doe</h4>
									<p class="card-text">Spécialité du stage</p>
									<a href="#" class="btn btn-primary">Page personnelle</a>
								</div>
							</div>
							<div class="card mr-5">
								<img class="card-img-top " src="img/studentsbg.jpg" alt="Card image">
								<div class="card-body">
									<h4 class="card-title">John Doe</h4>
									<p class="card-text">Spécialité du stage</p>
									<a href="#" class="btn btn-primary">Page personnelle</a>
								</div>
							</div>

						</div>
										
						
					</div>
				</div>
			</div>
		</div>
	</body>
</html>